...
<title>Sistem Akuntansi &mdash; Transaksi</title>
<?= $this->endSection() ?>
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td class="text-center" style="width:20%">
                              ...
                              <a href="<?= site_url('transaksi/'. $value->id_transaksi).'/edit' ?>" class="btn btn-warning btn-sm"><i class="fas fa-pencil-alt btn-small"></i>Edit</a>

                              <form action="<?=site_url('transaksi/'.$value->id_transaksi)?>" method="post" id="del<?=$value->id_transaksi?>" class="d-inline">
                              <?=csrf_field();?>
                              <input type="hidden" name="_method" value="DELETE">
                              <button class="btn btn-danger btn-sm" data-confirm="Hapus Data...? | Apakah anda Yakin..? - Karena Relasi Tabelnya akan terhapus Semua" data-confirm-yes="hapus(<?=$value->id_transaksi?>)"><i class="fas fa-trash"></i>Del</button>
                            </form>
                            </td>
                          </tr>
                          <?php endforeach; ?>
                          <!-- Debugging: Tampilkan data yang diterima dari kontroler -->
